...
def main() -> None:
    ...
    def _(_) -> None:
        clip.volume = volume_slider.value

    @loop_checkbox.on_update
    def _(_) -> None:
        clip.loop = loop_checkbox.value

    @positional_checkbox.on_update
    def _(_) -> None:
        clip.positional = positional_checkbox.value

    # Orbit the source around the origin; the audio node moves with it.
    while True:
        angle = time.time()
